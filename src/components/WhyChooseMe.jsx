const points = [
  {
    ...
    text: 'Mi formación en psicología me ayuda a entender cómo decide un cliente y a ordenar tu web para que genere confianza y acción, no solo para que se vea bonita.',
    icon: (
...
export default function WhyChooseMe() {
  return (
    <section className="difference-section bg-ink text-white">
      <div className="max-w-6xl mx-auto px-6 py-8 md:py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-4">
          {points.map((p) => (
            <div key={p.title} className="difference-card flex gap-4 rounded-3xl bg-white/5 border border-white/10 p-4 transition-all md:block md:p-5 duration-300 hover:-translate-y-1 hover:border-cyan-300/25 hover:bg-white/[.075]">
              <div className="w-10 h-10 shrink-0 rounded-2xl bg-brand-600/20 flex items-center justify-center md:w-12 md:h-12 md:mb-5">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="w-6 h-6 text-brand-300">
                  {p.icon}
                </svg>
              </div>
              <div>
                <h3 className="font-display text-base font-bold mb-1 md:text-lg md:mb-2">{p.title}</h3>
